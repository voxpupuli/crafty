...
RUN apt-get update \
  ...
  && curl -LR https://apt.voxpupuli.org/openvox8-release-ubuntu26.04.deb -o /tmp/openvox8-release-ubuntu26.04.deb \
  && dpkg -i /tmp/openvox8-release-ubuntu26.04.deb \
  && apt-get update \
  && apt-get install -y --no-install-recommends openvox-agent apache2 \
  && rm -rf /var/lib/apt/lists/* /tmp/openvox8-release-ubuntu26.04.deb
